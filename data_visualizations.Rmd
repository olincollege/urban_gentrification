...
```{r message=FALSE, warning=FALSE}
name_graph <- clean_data %>%
  ggplot() +
  geom_sf(aes(fill = NTAName), color = "#8f98aa") +
  theme_minimal() +
  ggtitle("Neighborhood Names")+ 
  theme(axis.text = element_blank(),
        panel.grid.major = element_line("transparent"),
        plot.title = element_text(family="DIN Condensed", size = 10, face = "bold", hjust=.5),
        legend.title = element_text(face="bold", family="DIN Condensed"), 
        legend.position = "right" ,
        legend.text = element_text(size = 6,face="bold", family="DIN Condensed"))
...
# Create Plotly plot
plot <- plot_ly(data = df, type = 'scatter', mode = 'markers', marker = list(size = 10), 
                hoverinfo = 'text',
                text = ~paste("Neighborhood: ", NTAName)) %>%
  layout(title = "Neighborhood Names",
         xaxis = list(title = ""),
         yaxis = list(title = ""),
         legend = list(title = "Neighborhood Names"))
