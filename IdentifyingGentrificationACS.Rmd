...
property_value_growth <- property_sales_2013_2023_grouped %>%
  ...
  filter(n_distinct(`sale price`) > 1) %>%
  select(block, lot, `sale price`, `sale date`) %>%
  group_by(block, lot)%>%
  ...
  select(block, lot, price_diff, date_diff)
```

```{r}
bayridge_appreciation <- property_value_growth %>%
  filter(block >= 5749 & block <= 6143)

bedstuy_appreciation <- property_value_growth %>%
  filter(block >= 1578 & block <= 2022)
```
